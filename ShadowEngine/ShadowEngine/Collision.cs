...
namespace ShadowEngine
{
    static public class Collision
    {
        ...
        public static void AddCollisionSegment(Vector2F first, Vector2F second, float colitionDistance)
        {
        ...
        public static void Update(Vector3 camaraPos)
        {
            counter++;
        ...
        public static Vector3 CheckCollisionWithMove(Vector3 camaraPos)
        {
            if (!ghostMode) //if colitions are not disabled
            {
                ...
                {
                    if (item.point.DistanceTo(camaraPos) < item.ColitionDistance)
                    {
